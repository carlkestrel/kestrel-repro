...
def _check_config_format(project_root: Path) -> dict:
    ...
    return {"name": "config_format", "status": "PASS", "message": "no config files, or they parse"}
...
def _check_plan_exists(plan_path: Path) -> dict:
    ...
    return {"name": "plan_exists", "status": "PASS", "message": f"plan {plan_path.name} found"}
...
def _check_rw(project_root: Path) -> dict:
    try:
        ...
        probe.write_text("ok")
        probe.unlink()
        return {"name": "rw", "status": "PASS", "message": f"writable: {project_root}"}
    except Exception as e:
        return {
            "name": "rw",
            "status": "FAIL",
...
def _check_gpu() -> dict:
    ...
    try:
        ...
        return {"name": "gpu", "status": "PASS", "message": f"{n}× {name}"}
    except ImportError:
        return {
            "name": "gpu",
            "status": "WARNING",
...
def _check_cuda_match(expected: str | None) -> dict:
    ...
    return {"name": "cuda_match", "status": "PASS", "message": f"CUDA {actual} matches expected"}
...
def _check_torch() -> dict:
    try:
        ...
        return {"name": "torch", "status": "PASS", "message": f"torch={version} cuda={cuda_v}"}
    except ImportError:
        return {
            "name": "torch",
            "status": "WARNING",
    ...
    try:
        ...
        for path in list(Path.cwd().rglob("*"))[:2000]:
            ...
            if any(part in SKIP_DIRS for part in path.parts):
                continue
            if path.suffix in {".pth", ".pt", ".bin", ".safetensors", ".png", ".jpg", ".pdf"}:
                continue
            try:
...
def _check_leftover_procs() -> dict:
    ...
    try:
        ...
        return {"name": "leftover_procs", "status": "PASS", "message": "ps not available"}
    pid_self = os.getpid()
    matches = []
    for line in out.splitlines():
        ...
        if pid == pid_self:
            continue
        cmd = parts[2]
        if "python" in parts[1].lower() and "reproctl" in cmd and Path.cwd().as_posix() in cmd:
            matches.append(pid)
    if matches:
        return {
            "name": "leftover_procs",
    ...
    return {"name": "leftover_procs", "status": "PASS", "message": "no leftover reproctl processes"}


def _check_state_file(project_root: Path) -> dict:
    p = project_root / ".repro" / "execution" / "execution_state.json"
    if not p.exists():
        return {"name": "state_file", "status": "PASS", "message": "no execution state yet"}
    try:
...
def _check_checkpoint(project_root: Path) -> dict:
    ...
    if not p.exists():
        return {"name": "checkpoint", "status": "PASS", "message": "no checkpoints directory yet"}
    for ck in p.glob("*"):
        try:
            sz = ck.stat().st_size
        except OSError:
        ...
        if sz < 100:
            try:
                ...
                continue
    return {"name": "checkpoint", "status": "PASS", "message": "no corrupt checkpoints detected"}
...
def run(
    *,
    ...
    checks = [
        ...
        _check_task_graph(plan_path)
        if plan_path is not None
        else {"name": "task_graph", "status": "PASS", "message": "skipped (no plan)"},
