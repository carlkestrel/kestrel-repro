...
(function() {
  ...
  function renderSearchResults(results, query) {
    ...
    results.forEach((result, index) => {
      ...
      titleDiv.innerHTML = highlightMatch(result.title || '无标题', query);
      
      const sectionDiv = document.createElement('div');
      sectionDiv.className = 'search-result-section';
      sectionDiv.textContent = (result.section || '文档') + ' — ' + (result.type || '页面').toUpperCase();
  ...
  function setupEventListeners() {
    ...
    document.querySelectorAll('h1[id], h2[id], h3[id], h4[id]').forEach(heading => {
      ...
      if (!anchor) {
        ...
        link.textContent = '¶';
        link.setAttribute('aria-label', '链接到章节');
